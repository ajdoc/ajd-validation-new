...
trait ExpressionBuilderTrait
{
    ...
    public static $BEHAVIORS = [
        'NORMAL'      => ExpressionBehavior::Normal,
        'OPTIMISTIC'  => ExpressionBehavior::Optimistic,
        'PESSIMISTIC' => ExpressionBehavior::Pessimistic,
    ];
    ...
    private function getBehaviorChars(): string
    {
        ...
        if ($behaviors === null) {
            $behaviors = implode('', ExpressionBehavior::values());
        }
    ...
    public function optimistic(): static
    {
        ...
        if (strpos($behaviors, $behavior) === false) {
            $this->buffer = ($behavior = ExpressionBehavior::Optimistic->value) . $this->buffer;
    ...
    public function pessimistic(): static
    {
        ...
        if (strpos($behaviors, $behavior) === false) {
            $this->buffer = ($behavior = ExpressionBehavior::Pessimistic->value) . $this->buffer;
            $this->tokens = [-1 => $behavior] + $this->tokens;
        }
    ...
    public function createRuleStatement(string $name, array $arguments): string
    {
        ...
        $statement = ExpressionEngine::createRuleStatement($name, $arguments);
